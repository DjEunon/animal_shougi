...
se=socketClient.soketClient("10.2.75.85")
...
while True:
    if se.myturn_now():
        ...
        str_temp=post_send_localserver(se.send_command("board"),se.get_whoami())
        if str_temp=="mv  ":
            bo=board_class.board_class(se.send_command("board"),se.get_whoami())
            tem_list.append(bo.killing_you())
            se.send_command("mv "+tem_list[0][1]+" "+tem_list[0][0])
            #se.send_command("mv "+tem_list[0]+" "+tem_list[1])
            break
        se.send_command(str_temp)
print("game_end")
